...
# Western Sydney (use Parramatta CBD as a representative point)
ws_lat <- -33.8136
ws_lng <- 151.0034

# Link two points
arc_pts <- geosphere::gcIntermediate(
  p1 = c(op_lng, op_lat),
  p2 = c(ws_lng, ws_lat),
  n = 200,            
  addStartEnd = TRUE
)
arc_df <- as.data.frame(arc_pts)

# Set icons
op_icon <- awesomeIcons(icon = "map-marker-alt", library = "fa",
                        markerColor = "blue", iconColor = "white")
ws_icon <- awesomeIcons(icon = "map-marker-alt", library = "fa",
...
intro_map_1 <- leaflet(
  ...
  addProviderTiles(providers$OpenStreetMap.Mapnik,
                   options = providerTileOptions(noWrap = TRUE)) %>%
  setView(lng = op_lng, lat = op_lat, zoom = 12) %>%
  addAwesomeMarkers(
    lng = op_lng, lat = op_lat,
    label = "Sydney Opera House",
    popup = sprintf("<b>Sydney Opera House</b><br/>lat: %.6f<br/>lng: %.6f", op_lat, op_lng),
    icon  = op_icon
  )
```
...
intro_map_2 <- leaflet(
  ...
  addAwesomeMarkers(
    ...
    popup = sprintf("<b>Western Sydney (Parramatta)</b><br/>lat: %.4f<br/>lng: %.4f", ws_lat, ws_lng),
    icon  = ws_icon
  )
```
...
intro_map_3 <- leaflet(
  ...
  addPolylines(
    data = arc_df, lng = ~lon, lat = ~lat,
    color = "green", weight = 3, opacity = 0.9, smoothFactor = 1
  )
```
