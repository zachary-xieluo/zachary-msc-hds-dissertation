...
cen_nsw_2021 <- read.csv("../data/cencus_nsw_2021.csv")
geo_nsw_2021 <- read.csv("../data/sa2_lga_lhd_nsw_2021.csv")
irsd_nsw_2021 <- read.csv("../data/irsd_nsw_2021.csv")
...
cen_wslhd_2021 <- cen_nsw_2021 %>%
  filter(in_wslhd == 1) %>%
  mutate(bachelor_below = adv_diploma + high_school_below,
         low_income_total = neg_nil_income + very_low_income + low_income,
         high_income_total = high_income + very_high_income)
```

```{r}
ws_prop <- cen_wslhd_2021 %>%
  summarise(
    # Education
    ...
    # Income
    prop_low_income_total  = sum(low_income_total,  na.rm = TRUE) / sum(denominator_income, na.rm = TRUE),
    prop_middle_income     = sum(middle_income,     na.rm = TRUE) / sum(denominator_income, na.rm = TRUE),
    prop_high_income_total = sum(high_income_total, na.rm = TRUE) / sum(denominator_income, na.rm = TRUE),
...
inc_df <- ws_prop %>%
  select(prop_low_income_total, prop_middle_income, prop_high_income_total, prop_unknown_income) %>%
  pivot_longer(everything(), names_to = "category", values_to = "prop") %>%
  mutate(category = recode(category,
                           prop_low_income_total  = "Low",
                           prop_middle_income     = "Middle",
                           prop_high_income_total = "High",
...
```{r}
make_plot <- function(df, title, base_col, cat_order, fade = FALSE, fade_alpha = 0.7){
...
p_edu    <- make_plot(edu_df,    "Education",       "#43978D",
                      cat_order = c("Bachelor or above","Below bachelor","Unknown"),
                      fade = FALSE)
p_occ    <- make_plot(occ_df,    "Occupation",      "#439755",
                      cat_order = c("Managers & Professionals","Everyone else","Unknown"),
                      fade = FALSE)
p_income <- make_plot(inc_df,    "Personal Income", "#D46C4E",
                      cat_order = c("Low","Middle","High","Unknown"),
                      fade = FALSE)
p_house  <- make_plot(hou_df,    "Housing Type",    "#F59E0B",
                      cat_order = c("Social housing","Non-social housing","Unknown"),
                      fade = FALSE)

wslhd_sec_1 <- (p_edu | p_occ) / (p_income | p_house) +
  plot_annotation(
    title = "Distribution of Socioeconomic Characteristics in WSLHD (2021)",
...
wslhd_sec_2 <- ggplot(
  data = map_wslhd_2021 %>%
    mutate(
      social_band = factor(
        if_else(prop_social_housing >= 0.047, "≥WSLHD (4.7%)", "<WSLHD (4.7%)"),
        levels = c("<WSLHD (4.7%)", "≥WSLHD (4.7%)")
      )
    ),
  ...
  scale_fill_manual(
          ...
          values = c("<WSLHD (4.7%)" = "#ffebc8","≥WSLHD (4.7%)" = "#F59E0B"
    ),
    drop     = FALSE,
    na.value = "grey85"
  ) +
  labs(
    caption  = "White areas represent non-residentials"
  ) +
  coord_sf(datum = NA) +
...
metro_healcons_1 <- ggplot(
  ...
  labs(x = NULL, y = NULL,
       fill = NULL,
       title = "Selected Health Condition Proportions: <span style='color:#43978D;'>NSW Metropolitan LHDs</span> vs <span style='color:#D46C4E;'>WSLHD</span> in 2021",
...
caption_html <- '<div style="background:white; padding:6px 10px; font-size:12px;">
  No colour areas represent non-residuals.
</div>'
