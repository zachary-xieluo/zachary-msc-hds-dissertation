...
# Local Government Areas (2011, NSW)
lga <- strayr::read_absmap("lga2011") |>
  dplyr::filter(state_name_2011 == "New South Wales")

# Statistical Areas Level 2 (2011, NSW)
...
sa2_lga_lhd_df <- sa2_to_lga |>
  left_join(sa2_to_lhd, by = "sa2_code_2011") |>
  arrange(sa2_code_2011)

# --- Optional sanity checks ---
stopifnot(!anyDuplicated(sa2_lga_lhd_df$sa2_code_2011))
stopifnot(!any(sapply(sa2_lga_lhd_df, inherits, "sfc")))

# --- Save output ---
# write.csv(sa2_lga_lhd_df, "sa2_lga_lhd_nsw_2011.csv", row.names = FALSE)
```

```{r}
# Filter WSLHD
sa2_lga_wslhd_2011 <- sa2_lga_lhd_df |>
  filter(nsw_lhd_name == "Western Sydney")

# Read all sa2s that wslhd covers
wslhd_sa2 <- read.csv("wslhd_sa2_2011.csv") %>%
  rename(sa2_name_2011 = sa2)

# Identify SA2s missing in sa2_lga_wslhd_2011 
missing_sa2 <- wslhd_sa2 %>%
  anti_join(sa2_lga_wslhd_2011, by = "sa2_name_2011")

# Extract full rows for the missing SA2s
rows_to_add <- sa2_lga_lhd_df %>%
  semi_join(missing_sa2, by = "sa2_name_2011")

# Append the missing SA2 rows
sa2_lga_wslhd_2011 <- sa2_lga_wslhd_2011 %>%
  bind_rows(rows_to_add) %>%
  distinct(sa2_name_2011, .keep_all = TRUE) 

# write.csv(sa2_lga_wslhd_2011, "sa2_lga_wslhd_2011.csv", row.names = FALSE)
```
