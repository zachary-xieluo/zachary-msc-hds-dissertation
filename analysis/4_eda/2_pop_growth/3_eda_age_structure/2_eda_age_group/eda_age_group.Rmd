...
age_wslhd_summary_long <- age_wslhd_summary %>%
  select(age_group, pop2011_as21, pop_2021) %>%
  pivot_longer(cols = c(pop2011_as21, pop_2021),
                      names_to = "year", values_to = "count") %>%
  mutate(year = recode(year,
                              pop2011_as21 = "2011 as-21",
                              pop_2021     = "2021"))
...
#   select(age_group4, pop2011_as21, pop_2021) %>%
#   pivot_longer(cols = c(pop2011_as21, pop_2021),
#                names_to = "year", values_to = "count") %>%
#   mutate(
#     year = recode(year,
...
wslhd_pop_1 <- ggplot(pop_wslhd_lga_long, aes(x = year, y = count, fill = year)) +
  ...
  labs(title = "Population by LGAs within WSLHD, <span style='color:#43978D;'>2011</span> and <span style='color:#D46C4E;'>2021</span> Census", 
       x = NULL, 
       y = NULL) +  
...
age_wslhd_lga_4g_long <- age_wslhd_lga_4g %>%
  select(lga_name_2021, age_group4, pop2011_as21, pop_2021, growth_rate) %>%
  pivot_longer(c(pop2011_as21, pop_2021), names_to = "year", values_to = "count") %>%
...
wslhd_pop_2 <- ggplot(age_wslhd_lga_4g_long, aes(x = age_group4, y = count, fill = year)) +
  ...
  labs(title = "Population by LGAs within WSLHD, <span style='color:#43978D;'>2011</span> and <span style='color:#D46C4E;'>2021</span> Census", 
       x = NULL, 
       y = NULL) +  
  scale_x_discrete(labels = c("0_19" = "0~19",
