...
cob_nsw_2021 <- read.csv("cobDetail_nsw_2021.csv") %>%
  rename(sa2_name_2021 = sa2)
lang_nsw_2021 <- read.csv("langDetail_nsw_2021.csv") %>%
  rename(sa2_name_2021 = sa2)
...
metro_cob_1 <- ggplot(cob_metro_lab, aes(x = nsw_lhd_name, y = prop, fill = origin)) +
  geom_col(width = 0.5) +
  geom_text(
    data = cob_metro_lab %>% filter(origin != "Not Stated"),
    aes(label = label),
  ...
  labs(x = NULL, y = NULL,
       fill = NULL,
       title = "<span style='color:#43978D;'>Born in Australia</span> vs <span style='color:#D46C4E;'>Born Overseas</span> (Proportions by Sydney Metropolitan LHDs, 2021 Census)") +
...
metro_lang_1 <- ggplot(lang_metro_lab, aes(x = nsw_lhd_name, y = prop, fill = origin)) +
  geom_col(width = 0.5) +
  geom_text(
    data = lang_metro_lab %>% filter(origin != "Not Stated"),
    aes(label = label),
    position = position_stack(vjust = 0.5),
    size = 4, lineheight = 0.9, color = "black") +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  ...
  labs(x = NULL, y = NULL,
       fill = NULL,
       title = "<span style='color:#439755;'>English</span> vs <span style='color:#F59E0B;'>Non-English</span> Used at Home (Proportions by Sydney Metropolitan LHDs, 2021 Census)") +
  theme_minimal(base_size = 12) +
  theme(
    plot.title     = element_markdown(size = 17, face = "bold", hjust = 0.5),
    axis.text.x    = element_text(size = 12, color = "black"),
    axis.text.y    = element_text(size = 16, color = "black"),
    legend.text    = element_text(size = 16, color = "black"),
    legend.position= "bottom",
...
wslhd_cob_1 <- ggplot(cob_lga_wslhd_lab, aes(x = lga_name_2021, y = prop, fill = origin)) +
  geom_col(width = 0.5) +
  geom_text(
    data = cob_lga_wslhd_lab %>% filter(origin != "Not Stated"),
    aes(label = label),
    position = position_stack(vjust = 0.5),
    size = 4, lineheight = 0.9, color = "black") +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  ...
  labs(x = NULL, y = NULL,
       fill = NULL,
       title = "<span style='color:#43978D;'>Born in Australia</span> vs <span style='color:#D46C4E;'>Born Overseas</span> (Proportions by LGAs within WSLHD, 2021 Census)") +
  theme_minimal(base_size = 12) +
  theme(
    plot.title     = element_markdown(size = 18, face = "bold", hjust = 0.5),
    axis.text.x    = element_text(size = 12, color = "black"),
...
wslhd_lang_1 <- ggplot(lang_lga_wslhd_lab, aes(x = lga_name_2021, y = prop, fill = origin)) +
  geom_col(width = 0.5) +
  geom_text(
    data = lang_lga_wslhd_lab %>% filter(origin != "Not Stated"),
    aes(label = label),
    position = position_stack(vjust = 0.5),
    size = 4, lineheight = 0.9, color = "black") +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  ...
  labs(x = NULL, y = NULL,
       fill = NULL,
       title = "<span style='color:#439755;'>English</span> vs <span style='color:#F59E0B;'>Non-English</span> Used at Home (Proportions by LGAs within WSLHD, 2021 Census)") +
  theme_minimal(base_size = 12) +
  theme(
    plot.title     = element_markdown(size = 18, face = "bold", hjust = 0.5),
...
wslhd_cob_2 <- ggplot(plot_dat_cob, aes(x = reorder(axis_label, count), y = count)) +
  ...
  labs(x = NULL, y = NULL,
       title = "Top 10 <span style='color:#D46C4E;'>Overseas</span> Countries of Birth in WSLHD, 2021 Census") +
  theme_minimal(base_size = 12) +
  theme(
...
wslhd_lang_2 <- ggplot(plot_dot_lang, aes(x = reorder(axis_label, count), y = count)) +
  ...
  labs(
    x = NULL, y = NULL,
    title = "Top 10 <span style='color:#F59E0B;'>Non-English</span> Languages Used at Home in WSLHD, 2021 Census"
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
cob_osea_high_wslhd_2021 <- cob_aus_osea_wslhd_2021 %>%
  mutate(
    ...
    osea_high_flag = case_when(
      ...
      osea_high_lgl                   ~ "> WSLHD average (47.0%)",
      TRUE                           ~ "≤ WSLHD average (47.0%)"
    ),
    oseahigh_flag = factor(osea_high_flag,
      levels = c("≤ WSLHD average (47.0%)", "> WSLHD average (47.0%)", "Excluded")
    )
  ) %>%
...
wslhd_cob_3 <- ggplot(map_osea_high_wslhd_2021) +
  ...
  scale_fill_manual(
    values = c(
      "≤ WSLHD average (47.0%)" = "#43978D",
      "> WSLHD average (47.0%)" = "#D46C4E",
      "Excluded"     = "#D0D7DE"      
    ),
    breaks = c("≤ WSLHD average (47.0%)", "> WSLHD average (47.0%)"), 
    name = NULL
  ) +
    labs(title = "SA2s Where the Overseas-Born Share Exceeds
         <br>WSLHD Average Rate of <span style='color:#D46C4E;'>47.0%</span>, 2021 Census",
       caption = "Grey areas represent non-residential zones") +
...
lang_noe_high_wslhd_2021 <- lang_eng_wslhd_2021 %>%
  mutate(
    ...
    noe_high_flag =case_when(
      ...
      noe_high_lgl                    ~ "> WSLHD average (50.7%)",
      TRUE                           ~ "≤ WSLHD average (50.7%)"
    ),
    oseahigh_flag = factor(noe_high_flag,
      levels = c("≤ WSLHD average (50.7%)", "> WSLHD average (50.7%)", "Excluded")
    )
  ) %>%
...
wslhd_lang_3 <- ggplot(map_noe_high_wslhd_2021) +
  ...
  scale_fill_manual(
    values = c(
      "≤ WSLHD average (50.7%)" = "#439755",
      "> WSLHD average (50.7%)" = "#F59E0B",
      "Excluded"     = "#D0D7DE"      
    ),
    breaks = c("≤ WSLHD average (50.7%)", "> WSLHD average (50.7%)"), 
    name = NULL
  ) +
    labs(title = "SA2s Where the Non-English Share Exceeds
         <br>WSLHD Average Rate of <span style='color:#F59E0B;'>50.7%</span>, 2021 Census",
       caption = "Grey areas represent non-residential zones") +
  theme_minimal(base_size = 13) +
  theme(
    panel.grid      = element_blank(),
