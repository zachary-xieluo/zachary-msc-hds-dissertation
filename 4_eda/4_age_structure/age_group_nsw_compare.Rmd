...
```{r}
# --- 0) Helpers: standardize SA2 strings for join keys (keep 2021 names as-is for display) ---
...
# --- 2.2) patch non-spatial rows into mapping (self-map, weight=1) ---
patch_rows <- tibble(
...
# --- 3) Pivot to long (underscore style like 0_9, 100_plus) ---
age_2011_long <- age_nsw_2011 %>%
  ...
  select(sa2_name_2011, key_2011, age_group, n_2011)
...
age_2021_long <- age_nsw_2021 %>%
  ...
  select(sa2_name_2021, key_2021, age_group, n_2021)

# --- 4) Allocate 2011 counts to 2021 SA2 using mapping weights, per age group ---
age_pairs <- nsw_sa2_mapping_fix %>%
  select(key_2011, key_2021, weight_norm_from_2011) %>%
  ...
  mutate(
    n_2011 = coalesce(n_2011, 0),
    weight_norm_from_2011 = coalesce(weight_norm_from_2011, 0),
    contrib_2011_to_2021 = n_2011 * weight_norm_from_2011
  )

age2011_as21 <- age_pairs %>%
  group_by(sa2_name_2021, key_2021, age_group) %>%
  summarise(n2011_as21 = sum(contrib_2011_to_2021, na.rm = TRUE), .groups = "drop")

# --- 5) Combine with 2021 counts and compute changes ---
age_sa2_growth <- age_2021_long %>%
  select(sa2_name_2021, key_2021, age_group, n_2021) %>%
  full_join(age2011_as21, by = c("sa2_name_2021","key_2021","age_group")) %>%
  mutate(
    n_2021      = coalesce(n_2021, 0),
    n2011_as21  = coalesce(n2011_as21, 0),
...
total_2011_raw  <- age_2011_long  %>% summarise(sum = sum(n_2011,      na.rm = TRUE)) %>% pull(sum)
total_2011_as21 <- age2011_as21   %>% summarise(sum = sum(n2011_as21,  na.rm = TRUE)) %>% pull(sum)
