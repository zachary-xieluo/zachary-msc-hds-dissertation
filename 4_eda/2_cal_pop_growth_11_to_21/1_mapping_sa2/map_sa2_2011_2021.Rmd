---
Description: Mapping SA2 list from 2011 to 2021

Approach: 

1. Use ABS correspondences with 2016 as the bridge (no direct 2011→2021 table).

2. Weights: for 2011→2016 prefer `RATIO`; for 2016→2021 use `RATIO_FROM_TO`

3. Chain the two legs: `w_11_21 = w_11_16 * w_16_21`, then aggregate by (SA2_2011, sa2_2021) to get raw weights.

4. Normalise two ways:
   - Sources of a 2021 SA2: `weight_norm_to_2021 = weight_raw / sum(weight_raw)` grouped by SA2_2021
   - Splits of a 2011 SA2: `weight_norm_from_2011 = weight_raw / sum(weight_raw)` grouped by SA2_2011 

5. Keep only pairs whose 2021 SA2 is in NSW by joining on `sa2_code_2021`.

6. Append all original 2021 mapping columns by joining on `sa2_code_2021`.
