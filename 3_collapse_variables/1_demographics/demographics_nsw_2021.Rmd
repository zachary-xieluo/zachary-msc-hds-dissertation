...
lang_nsw_2021 <- read.csv("lang_nsw_2021.csv")
```
...
lang_nsw_2021 <- lang_nsw_2021 %>%
  rename(
    english = English,
    supplementary_codes_lang = "Supplementary.codes",
    not_stated_lang = "Not.stated"
  )
```
...
pop_nsw_2021 <- pop_nsw_2021 %>%
  ...
  left_join(lang_nsw_2021, by = "sa2")
```
...
non_english_cols <- setdiff(
  names(select(lang_nsw_2021, where(is.numeric))),
  c("english", "supplementary_codes_lang", "not_stated_lang")
)
```
