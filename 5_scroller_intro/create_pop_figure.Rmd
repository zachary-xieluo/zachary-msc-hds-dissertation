...
metro_pop_1 <- ggplot(
  ...
  scale_y_continuous(labels = scales::label_comma()) +
  labs(title = NULL, x = NULL, y = "Population") +             
  theme_minimal(base_size = 11) +
  theme(
...
bbox_pad_plot <- function(sf_row,
  ...
  p <- ggplot() +
    # LHD fill
    geom_sf(data = sf_row, fill = fill_col, color = NA) +
    ...
    #   else NULL } +
    # SA2 borders
    geom_sf(data = sa2_sub, fill = NA,
            color = sa2_border_col, linewidth = sa2_border_size) +
    # LHD outer border on top
    geom_sf(data = sf_row, fill = NA,
            color = lhd_border_col, linewidth = lhd_border_size) +
    coord_sf(
      xlim   = c(cx - R, cx + R),
      ylim   = c(cy - R, cy + R),
      expand = FALSE,
...
bbox_pad_plot_hl <- function(sf_row,
  ...
  sa2_sub <- sf::st_filter(sa2_metro_pl, sf_row, .predicate = sf::st_intersects) |>
    dplyr::mutate(.key = normalize_key(sa2_name_2021))

  sa2_hl_general <- dplyr::filter(sa2_sub, .key %in% setdiff(hl_keys_all, hl_keys_special))
  sa2_hl_special <- dplyr::filter(sa2_sub, .key %in% hl_keys_special)
  ...
  if (show_lga && exists("lga_metro_pl") && !is.null(lga_metro_pl)) {
    lga_sub <- sf::st_filter(lga_metro_pl, sf_row, .predicate = sf::st_intersects)
    if (nrow(lga_sub) > 0) {
  ...
  # step 1: general highlights (orange)
  if (paint_general && nrow(sa2_hl_general) > 0) {
    p <- p + geom_sf(data = sa2_hl_general, fill = general_fill, color = NA, alpha = hl_alpha)
  }

  # step 2: special highlights (brown) — drawn after general to override
  if (paint_special && nrow(sa2_hl_special) > 0) {
    p <- p + geom_sf(data = sa2_hl_special, fill = special_fill, color = NA, alpha = hl_alpha)
  }
...
metro_pop_5 <- patchwork::wrap_plots(plots_2, ncol = 3)
